...
use OCA\MigrateToInfiniteScale\Command\MigrateGroups;
...
class MigrateGroupsTest extends \Test\TestCase {
	/** @var Migration */
	private Migration $migration;
	/** @var MigrateGroups */
	private MigrateGroups $migrateGroupsCommand;

	protected function setUp(): void {
		$this->migration = $this->createMock(Migration::class);

		$this->groupsCommand = new MigrateGroups($this->migration);
		$this->groupsCommand->setApplication(new Application());
		$this->commandTester = new CommandTester($this->groupsCommand);
	}

	public function testMigrate(): void {
		$stateGroups = $this->createMock(StateMigrateGroups::class);
		$stateGroups->method('associatedCommand')->willReturn('migrate:to-ocis:migrate:groups');
		...
		$this->migration->expects($this->exactly(2))
			->method('getState')
			->willReturnCallback(function () use ($stateGroups, $stateNotFinish) {
				static $calls = -1;
				$states = [$stateGroups, $stateNotFinish];
				$calls++;
				return $states[$calls];
			});
	...
	public function testSkip(): void {
		...
		$this->commandTester->setInputs(['FakeAdminPassword']);
		self::assertSame(0, $this->commandTester->execute(['ocis-admin' => 'admin', '--skip' => true]));
		self::assertStringContainsStringIgnoringCase('Continue the migration with', $this->commandTester->getDisplay());
	}
}
