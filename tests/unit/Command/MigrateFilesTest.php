...
use OCA\MigrateToInfiniteScale\Command\MigrateFiles;
...
class MigrateFilesTest extends \Test\TestCase {
	/** @var Migration */
	...
	protected function setUp(): void {
		...
		$this->filesCommand = new MigrateFiles($this->migration);
		$this->filesCommand->setApplication(new Application());
		$this->commandTester = new CommandTester($this->filesCommand);
	}

	public function testMigrate(): void {
		$stateFiles = $this->createMock(StateMigrateFiles::class);
		$stateFiles->method('associatedCommand')->willReturn('migrate:to-ocis:migrate:files');
		...
		$this->migration->expects($this->exactly(2))
			->method('getState')
			->willReturnCallback(function () use ($stateFiles, $stateNotFinish) {
				static $calls = -1;
				$states = [$stateFiles, $stateNotFinish];
				$calls++;
				return $states[$calls];
			});
	...
	public function testSkip(): void {
		...
		$this->migration->expects($this->once())->method('loadState');
		$this->migration->expects($this->never())->method('saveState');
		$this->migration->expects($this->once())->method('getState')->willReturn($stateFiles);
		$this->migration->expects($this->never())->method('runMigration');
