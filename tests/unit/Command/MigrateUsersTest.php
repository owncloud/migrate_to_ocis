...
use OCA\MigrateToInfiniteScale\Command\MigrateUsers;
...
class MigrateUsersTest extends \Test\TestCase {
	/** @var Migration */
	private Migration $migration;
	/** @var MigrateUsers */
	private MigrateUsers $migrateUsersCommand;

	protected function setUp(): void {
		$this->migration = $this->createMock(Migration::class);

		$this->usersCommand = new MigrateUsers($this->migration);
		$this->usersCommand->setApplication(new Application());
		$this->commandTester = new CommandTester($this->usersCommand);
	}

	public function testMigrate(): void {
		$stateUsers = $this->createMock(StateMigrateUsers::class);
		$stateUsers->method('associatedCommand')->willReturn('migrate:to-ocis:migrate:users');
		...
		$this->migration->expects($this->exactly(2))
			->method('getState')
			->willReturnCallback(function () use ($stateUsers, $stateNotFinish) {
				static $calls = -1;
				$states = [$stateUsers, $stateNotFinish];
				$calls++;
				return $states[$calls];
			});
